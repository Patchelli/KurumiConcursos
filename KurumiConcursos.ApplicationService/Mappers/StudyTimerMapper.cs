...
public sealed class StudyTimerMapper : IStudyTimerMapper
{
    ...
    {
        s.JourneyId = r.JourneyId;
        s.KnowledgeAreaId = r.KnowledgeAreaId;
        s.SyllabusNodeId = r.SyllabusNodeId;
        s.Mode = r.Mode;
        s.Phase = r.Phase;
        ...
        s.FocusMinutes = r.FocusMinutes;
        s.ShortBreakMinutes = r.ShortBreakMinutes;
        s.LongBreakMinutes = r.LongBreakMinutes;
        s.Cycles = r.Cycles;
        s.CurrentCycle = r.CurrentCycle;
        s.RunningSince = r.IsRunning ? now : null;
        s.LastUpdateDate = now;
    }

    public StudyTimerResponse DomainToDtoResponse(StudyTimerSession s, DateTimeOffset now)
    {
        var elapsed = s.IsRunning && s.RunningSince.HasValue
            ? Math.Max(0, (int)(now - s.RunningSince.Value).TotalSeconds)
            : 0;
        var limit = s.Mode == EStudyTimerMode.Pomodoro
            ? (s.Phase switch
            {
