...
{
    Task<IList<NextcloudEntryResponse>> BrowseAsync(string? path, UserCredential credential, CancellationToken cancellationToken);
    Task<IList<PrivateMaterialResponse>> FindAllAsync(long topicId, UserCredential credential);
    Task<PrivateMaterialResponse?> LinkAsync(long topicId, PrivateMaterialLinkRequest request, UserCredential credential, CancellationToken cancellationToken);
    Task<bool> DeleteAsync(long topicId, long materialId, UserCredential credential);
    Task<PrivateMaterialFile?> OpenAsync(long topicId, long materialId, UserCredential credential, CancellationToken cancellationToken);
    Task<IList<PrivateMaterialResponse>> FindAllByAreaAsync(long areaId, UserCredential credential);
    Task<PrivateMaterialResponse?> LinkToAreaAsync(long areaId, PrivateMaterialLinkRequest request, UserCredential credential, CancellationToken cancellationToken);
    Task<bool> DeleteByAreaAsync(long areaId, long materialId, UserCredential credential);
    Task<PrivateMaterialFile?> OpenByAreaAsync(long areaId, long materialId, UserCredential credential, CancellationToken cancellationToken);
}
